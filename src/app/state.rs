...
use crate::usecases::ui::{ParamUpdate, ModulationAction, UIActions};
use super::VardaApp;
    ...
    pub(crate) fn apply_deck_and_effect_actions(
        ...
        if let Some((ch_idx, gen_idx)) = actions.shader_to_add {
            ...
            if gen_idx < generators.len() {
                ...
                match crate::Deck::new(context, shader.clone(), super::RENDER_WIDTH, super::RENDER_HEIGHT) {
                    Ok(deck) => {
                        if let Some(ch) = mixer.channel_mut(ch_idx) {
        ...
        if let Some((ch_idx, path)) = actions.image_to_add.take() {
            match crate::Deck::new_from_image(context, &path, super::RENDER_WIDTH, super::RENDER_HEIGHT) {
                Ok(deck) => {
                    if let Some(ch) = mixer.channel_mut(ch_idx) {
        ...
        if let Some((ch_idx, path)) = actions.video_to_add.take() {
            match crate::Deck::new_from_video(context, &path, super::RENDER_WIDTH, super::RENDER_HEIGHT) {
                Ok(deck) => {
                    if let Some(ch) = mixer.channel_mut(ch_idx) {
                        let name = deck.source_name().to_string();
                        let idx = ch.add_deck(deck);
        ...
        if let Some((ch_idx, color)) = actions.solid_color_to_add.take() {
            match crate::Deck::new_solid_color(context, color, super::RENDER_WIDTH, super::RENDER_HEIGHT) {
                Ok(deck) => {
                    if let Some(ch) = mixer.channel_mut(ch_idx) {
                        let name = deck.source_name().to_string();
                        let idx = ch.add_deck(deck);
        ...
        if let Some((ch_idx, deck_idx, filter_idx)) = actions.effect_to_add {
            ...
            if filter_idx < filters.len() {
                ...
                if let Some(ch) = mixer.channel_mut(ch_idx) {
                    if deck_idx < ch.decks.len() {
                        match crate::Effect::new(context, filter_shader.clone()) {
                            Ok(effect) => {
        ...
        if let Some((ch_idx, filter_idx)) = actions.ch_effect_to_add {
            ...
            if filter_idx < filters.len() {
                let filter_shader = filters[filter_idx].clone();
                if let Some(ch) = mixer.channel_mut(ch_idx) {
                    match crate::Effect::new_with_format(context, filter_shader.clone(), context.texture_format) {
                        Ok(effect) => {
        ...
        if let Some(filter_idx) = actions.master_effect_to_add {
            ...
            if filter_idx < filters.len() {
                ...
                match crate::Effect::new_with_format(context, filter_shader, context.texture_format) {
                    Ok(effect) => {
