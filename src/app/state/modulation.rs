        ...
        for action in &actions.modulation_actions {
            match action {
                ModulationAction::AddLFO { waveform, frequency } => {
                    ...
                    let idx = mixer.modulation_mut().add_source(source);
                ...
                ModulationAction::RemoveSource { idx } => {
                    mixer.modulation_mut().remove_source(*idx);
            ...
            ModulationAction::UpdateLFOFrequency { idx, frequency } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::LFO { frequency: ref mut f, .. } = mixer.modulation_mut().sources[*idx] {
                        *f = *frequency;
                    }
                }
            }
            ModulationAction::UpdateLFOWaveform { idx, waveform } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::LFO { waveform: ref mut w, .. } = mixer.modulation_mut().sources[*idx] {
                        *w = *waveform;
                    }
                }
            }
            ModulationAction::UpdateLFOPhase { idx, phase } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::LFO { phase: ref mut p, .. } = mixer.modulation_mut().sources[*idx] {
                        *p = *phase;
                    }
                }
            }
            ModulationAction::UpdateLFOAmplitude { idx, amplitude } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::LFO { amplitude: ref mut a, .. } = mixer.modulation_mut().sources[*idx] {
                        *a = *amplitude;
                    }
                }
            }
            ModulationAction::UpdateLFOBipolar { idx, bipolar } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::LFO { bipolar: ref mut b, .. } = mixer.modulation_mut().sources[*idx] {
                        *b = *bipolar;
                    }
                }
            }
            ModulationAction::UpdateAudioSmoothing { idx, smoothing } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { smoothing: ref mut s, .. } = mixer.modulation_mut().sources[*idx] {
                        *s = *smoothing;
                    }
                }
            }
            ModulationAction::UpdateAudioFreqLow { idx, freq_low } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { freq_low: ref mut fl, .. } = mixer.modulation_mut().sources[*idx] {
                        *fl = *freq_low;
                    }
                }
            }
            ModulationAction::UpdateAudioFreqHigh { idx, freq_high } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { freq_high: ref mut fh, .. } = mixer.modulation_mut().sources[*idx] {
                        *fh = *freq_high;
                    }
                }
            }
            ModulationAction::UpdateAudioGain { idx, gain } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { gain: ref mut g, .. } = mixer.modulation_mut().sources[*idx] {
                        *g = *gain;
                    }
                }
            }
            ModulationAction::UpdateAudioPreset { idx, preset } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { freq_low: ref mut fl, freq_high: ref mut fh, .. } = mixer.modulation_mut().sources[*idx] {
            ...
            ModulationAction::UpdateAudioSource { idx, source_id } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { source_id: ref mut sid, .. } = mixer.modulation_mut().sources[*idx] {
                        *sid = *source_id;
                    }
                }
            }
            ModulationAction::UpdateAudioMode { idx, mode } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { mode: ref mut m, .. } = mixer.modulation_mut().sources[*idx] {
                        *m = *mode;
                    }
                }
            }
            ModulationAction::UpdateAudioNoiseGate { idx, noise_gate } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::AudioBand { noise_gate: ref mut ng, .. } = mixer.modulation_mut().sources[*idx] {
                        *ng = *noise_gate;
                    }
                }
            }
            ...
            ModulationAction::UpdateADSRAttack { idx, attack } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::ADSR { attack: ref mut a, .. } = mixer.modulation_mut().sources[*idx] {
                        *a = *attack;
                    }
                }
            }
            ModulationAction::UpdateADSRDecay { idx, decay } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::ADSR { decay: ref mut d, .. } = mixer.modulation_mut().sources[*idx] {
                        *d = *decay;
                    }
                }
            }
            ModulationAction::UpdateADSRSustain { idx, sustain } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::ADSR { sustain: ref mut s, .. } = mixer.modulation_mut().sources[*idx] {
                        *s = *sustain;
                    }
                }
            }
            ModulationAction::UpdateADSRRelease { idx, release } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::ADSR { release: ref mut r, .. } = mixer.modulation_mut().sources[*idx] {
                        *r = *release;
                    }
                }
            }
            ModulationAction::TriggerADSR { idx } => {
                mixer.modulation_mut().trigger_adsr(*idx);
            }
            ModulationAction::ReleaseADSR { idx } => {
                mixer.modulation_mut().release_adsr(*idx);
            }
            ...
            ModulationAction::UpdateStepRate { idx, rate } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::StepSequencer { rate: ref mut r, .. } = mixer.modulation_mut().sources[*idx] {
                        *r = *rate;
                    }
                }
            }
            ModulationAction::UpdateStepInterpolation { idx, interpolation } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::StepSequencer { interpolation: ref mut interp, .. } = mixer.modulation_mut().sources[*idx] {
            ...
            ModulationAction::UpdateStepBipolar { idx, bipolar } => {
                if *idx < mixer.modulation_mut().sources.len() {
                    if let ModulationSource::StepSequencer { bipolar: ref mut b, .. } = mixer.modulation_mut().sources[*idx] {
                        *b = *bipolar;
                    }
                }
            }
            ModulationAction::AssignModOnMod { target_source_idx, param_name, modulator_idx, amount } => {
                mixer.modulation_mut().assign_mod_on_mod(*target_source_idx, param_name, *modulator_idx, *amount);
            ...
            ModulationAction::RemoveModOnMod { target_source_idx, param_name } => {
                mixer.modulation_mut().clear_mod_on_mod(*target_source_idx, param_name);
            ...
            ModulationAction::AssignModulation { ch_idx, deck_idx, param_name, source_idx, amount } => {
                mixer.modulation_mut().assign(&format!("ch{}_deck{}:{}", ch_idx, deck_idx, param_name), *source_idx, *amount, None);
            ...
            ModulationAction::RemoveAssignment { ch_idx, deck_idx, param_name, .. } => {
                mixer.modulation_mut().clear_assignments(&format!("ch{}_deck{}:{}", ch_idx, deck_idx, param_name));
            ...
            ModulationAction::AssignEffectModulation { ch_idx, deck_idx, effect_idx, param_name, source_idx, amount } => {
                ...
                mixer.modulation_mut().assign(&key, *source_idx, *amount, None);
