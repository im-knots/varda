...
impl VardaApp {
    ...
    pub fn cmd_update_surface_vertices(&mut self, uuid: &str, vertices: Vec<[f32; 2]>) -> CommandResult {
    ...
    pub fn cmd_insert_surface_vertex(&mut self, uuid: &str, after_vert_idx: usize, position: [f32; 2]) -> CommandResult {
        if let Some((_, surface)) = self.output.surface_manager.find_by_uuid_mut(uuid) {
    ...
    pub fn cmd_update_surface_contour_vertices(&mut self, uuid: &str, contour: usize, vertices: Vec<[f32; 2]>) -> CommandResult {
        if let Some((_, surface)) = self.output.surface_manager.find_by_uuid_mut(uuid) {
            if contour == 0 {
                if let Some(ref mut hint) = surface.circle_hint {
    ...
    pub fn cmd_assign_surface_to_output_by_idx(&mut self, output_idx: usize, surface_uuid: &str) -> CommandResult {
    ...
    pub fn cmd_unassign_surface_from_output_by_idx(&mut self, output_idx: usize, assignment_idx: usize) -> CommandResult {
        if let Some(output) = self.output.outputs.get_mut(output_idx) {
            let assignments = output.surface_assignments_mut();
