...
impl VardaApp {
    pub(crate) fn cmd_create_sequence(&mut self) -> CommandResult {
        ...
        self.mixer
            ...
            .push(crate::mixer::TransitionSequence::new(format!(
                "Sequence {}",
                n
            )));
        CommandResult::Ok
    ...
    pub(crate) fn cmd_delete_sequence(&mut self, idx: usize) -> CommandResult {
        if idx < self.mixer.transition_sequences().len() {
            ...
            CommandResult::Err {
                ...
                message: "Sequence not found".into(),
            }
        }
    }
    ...
    pub(crate) fn cmd_add_fade_step(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            seq.steps.push(crate::mixer::TransitionStep {
                kind: crate::mixer::StepKind::Fade {
                    from_ch,
                    to_ch,
    ...
    pub(crate) fn cmd_add_wait_step(&mut self, seq_idx: usize) -> CommandResult {
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            seq.steps.push(crate::mixer::TransitionStep {
                kind: crate::mixer::StepKind::Wait {
                    duration: crate::channel::DurationSpec::Seconds(2.0),
                },
            });
            CommandResult::Ok
    ...
    pub(crate) fn cmd_add_goto_step(&mut self, seq_idx: usize, step_index: usize) -> CommandResult {
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            seq.steps.push(crate::mixer::TransitionStep {
                kind: crate::mixer::StepKind::GoTo { step_index },
            });
            CommandResult::Ok
        } else {
            CommandResult::Err {
    ...
    pub(crate) fn cmd_set_step_duration(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                match &mut step.kind {
                    crate::mixer::StepKind::Fade { duration, .. }
    ...
    pub(crate) fn cmd_set_step_transition_shader(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                if let crate::mixer::StepKind::Fade {
                    transition_shader, ..
                } = &mut step.kind
                {
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                match &mut step.kind {
                    ...
                    | crate::mixer::StepKind::Wait { duration } => {
                        *duration =
                            crate::channel::DurationSpec::from_value_unit(duration.value(), unit);
                    }
                    _ => {}
                }
                CommandResult::Ok
            ...
            if let Some(step) = seq.steps.get_mut(step_idx) {
                match &mut step.kind {
                    ...
                    | crate::mixer::StepKind::Wait { duration } => {
                        ...
                        *duration = crate::channel::DurationSpec::from_value_unit(
                            duration.value(),
                            next_unit,
                        );
                    }
    ...
    pub(crate) fn cmd_set_step_from_ch(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                if let crate::mixer::StepKind::Fade { from_ch, .. } = &mut step.kind {
                    *from_ch = ch;
                }
                CommandResult::Ok
            } else {
    ...
    pub(crate) fn cmd_set_step_to_ch(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                if let crate::mixer::StepKind::Fade { to_ch, .. } = &mut step.kind {
                    *to_ch = ch;
                }
                CommandResult::Ok
            } else {
    ...
    pub(crate) fn cmd_set_goto_target(
        ...
        if let Some(seq) = self.mixer.transition_sequences_mut().get_mut(seq_idx) {
            if let Some(step) = seq.steps.get_mut(step_idx) {
                if let crate::mixer::StepKind::GoTo { step_index } = &mut step.kind {
                    *step_index = target;
                }
                CommandResult::Ok
            } else {
