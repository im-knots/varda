...
pub(crate) mod tests {
        ...
        EngineState {
            mixer: MixerSnapshot {
                channels: vec![ChannelSnapshot {
                    idx: 0, uuid: "ch-001".into(), name: "Channel A".into(),
                    opacity: 1.0, blend_mode: BlendMode::Normal,
                    decks: vec![DeckSnapshot {
                        idx: 0, uuid: "dk-001".into(), name: "Sine".into(),
                        opacity: 1.0, effective_opacity: 1.0,
                        blend_mode: BlendMode::Normal, solo: false, mute: false,
                        scaling_mode: None,
                        generator: ShaderParamsSnapshot { shader_name: "Sine".into(), params: vec![] },
                        effects: vec![], video_playback: None, auto_transition: None, fps: 60.0,
                    }],
                    effects: vec![], render_time_ms: 0.5, active_deck_count: 1,
                }],
                crossfader: 0.5, auto_crossfade_active: false, auto_crossfade_progress: 0.0,
                master_effects: vec![], active_transition_name: None,
                transition_names: vec!["Dissolve".into()], sequences: vec![],
            },
            audio: AudioSnapshot {
            ...
            modulation: ModulationSnapshot {
                sources: vec![], current_values: Default::default(), assignments: Default::default(),
            },
            outputs: OutputSnapshot {
                windows: vec![OutputWindowSnapshot {
                    uuid: "out-001".into(), name: "Output 1".into(),
                    target_label: "HDMI-1".into(), is_on_display: true,
                    surface_assignments: vec![], calibration_mode: false,
                }],
                surfaces: vec![SurfaceSnapshot {
                    uuid: "srf-001".into(), name: "Main".into(),
                    vertices: vec![[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0]],
                    extra_contours: vec![], source: OutputSource::Master,
            ...
            midi: MidiSnapshot { devices: vec![], mappings: vec![], learn_active: false, learn_target: None },
            cameras: CameraSnapshot { devices: vec![("FaceTime".into(), 0u32)] },
            clock: ClockSnapshot {
                bpm: Some(120.0), beat_phase: 0.0, source_label: "Audio".into(),
                device_name: None, active: true, detected_midi_sources: vec![],
                osc_active: false, osc_bpm: None, audio_bpm: Some(120.0),
                preference_label: "Auto".into(), preference_force_device_id: None, manual_bpm: None,
            },
    ...
    fn test_read_state_not_initialized() {
        ...
        assert!(matches!(read_state(&lock), Err(StateReadError::NotInitialized)));
    }
