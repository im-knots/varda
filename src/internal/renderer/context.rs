...
/// What a headless target can carry: the presentation a request resolves to on
/// it, and every mode with the reason it cannot be delivered. Supplied by the
/// caller, because what a recording or stream can carry is decided by the
/// delivery path, which the renderer does not name.
pub type PresentationPlanner =
    fn(&OutputTarget, PresentationRequest) -> (ResolvedPresentation, Vec<ModeAvailability>);
...
pub struct HeadlessOutput {
    ...
    pub height: u32,
    /// Resolves presentation requests against the current target.
...
impl HeadlessOutput {
    ...
    pub fn new(
        ...
        let presentation_request = PresentationRequest::default();
        let (resolved_presentation, mode_availability) = planner(&target, presentation_request);
    ...
    pub fn set_presentation_request(
        ...
        let (resolved, mode_availability) = (self.planner)(&self.target, request);
        self.mode_availability = mode_availability;
...
mod tests {
    use super::{
        ...
        select_surface_presentation,
    };

    /// A planner for tests that set the resolved presentation themselves.
    fn default_plan(
        _: &OutputTarget,
        _: PresentationRequest,
    ) -> (
        ResolvedPresentation,
        Vec<crate::engine::value::render::ModeAvailability>,
    ) {
        (ResolvedPresentation::default(), Vec::new())
    }
    ...
    /// fails the build when one exceeds the other by more than 200 bytes. Field
    /// sizes differ by platform, so a gap under the threshold on one can be over
    /// it on another. This catches that in the test suite, which runs everywhere,
    /// rather than in a per-platform lint job.
    #[test]
