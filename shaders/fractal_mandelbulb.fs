/*{
    "DESCRIPTION": "Mandelbulb - Raymarched 3D fractal explorer with flythrough camera, fog, and orbit trap coloring",
    "CREDIT": "Varda VJ",
    "ISFVSN": "2.0",
    "CATEGORIES": ["Generator", "Generative", "3D", "Fractal"],
    "INPUTS": [
        {"NAME": "iterations", "TYPE": "float", "DEFAULT": 8.0, "MIN": 3.0, "MAX": 16.0, "LABEL": "Iterations"},
        {"NAME": "power", "TYPE": "float", "DEFAULT": 8.0, "MIN": 2.0, "MAX": 16.0, "LABEL": "Power"},
        {"NAME": "bailout", "TYPE": "float", "DEFAULT": 4.0, "MIN": 1.5, "MAX": 10.0, "LABEL": "Bailout"},
        ...
        {"NAME": "color1", "TYPE": "color", "DEFAULT": [0.6, 0.4, 0.2, 1.0], "LABEL": "Color Warm"},
        {"NAME": "color2", "TYPE": "color", "DEFAULT": [0.3, 0.5, 0.9, 1.0], "LABEL": "Color Cool"},
...
vec2 mandelbulbDE(vec3 pos) {
    ...
    r = length(z);
    return vec2(0.5 * log(max(r, 1e-10)) * r / max(dr, 1e-10), trap);
}
...
void main() {
    ...
    if (hit) {
        ...
        float t;
        if (cm < 0.5) {
            t = clamp(sqrt(hitInfo.y) * 0.8, 0.0, 1.0);
        } else if (cm < 1.5) {
            t = clamp(dist * 0.1, 0.0, 1.0);
        } else {
