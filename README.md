...
- **Sources**: video (HAP GPU-native + ffmpeg), cameras, GLSL shaders (generators/filters, ISF-style metadata), NDI, SRT, HLS, DASH, RTMP/RTMPS, Syphon (macOS receive), screen and window capture (macOS), program taps of Varda's own output, images, and html/css/js sources (Servo)
