...
- **Sources**: video (HAP GPU-native + ffmpeg), cameras, GLSL shaders (generators/filters, ISF-style metadata, compute shaders), NDI, SRT, HLS, DASH, RTMP/RTMPS, Syphon (macOS receive), Spout (Windows receive), screen and window capture, program taps of Varda's own output, images, and html/css/js sources (Servo)
