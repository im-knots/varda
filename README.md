...
- **Sources**: video (HAP GPU-native + ffmpeg), cameras, GLSL shaders (generators/filters, ISF-style metadata, compute shaders), NDI, SRT, HLS, DASH, RTMP/RTMPS, Syphon (macOS receive), screen and window capture, program taps of Varda's own output, images, and html/css/js sources (Servo)
...
- **Network I/O**: NDI, SRT, HLS, DASH, and RTMP/RTMPS send/receive, with HDR10 and HLG on the HEVC/AV1 streaming paths. Zero copy Syphon send and receive on macOS.
